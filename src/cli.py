...
def preprocess_song(song_input, mdx_model_params, song_id, input_type, progress=None):
    keep_orig = False
    if input_type == 'yt':
...
def song_cover_pipeline(song_input, voice_model, pitch_change, keep_files,
    ...
    try:
        if not song_input or not voice_model:
            raise_exception('Ensure that the song input field and voice model field is filled.')

        display_progress('[~] Starting AI Cover Generation Pipeline...')
        ...
        if pitch_change_all != 0:
            display_progress('[~] Applying overall pitch change', 0.85)
