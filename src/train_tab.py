...
import os
import gradio as gr
from .training import *  # Assuming this imports necessary functions
        ...
        with gr.Row():
            with gr.Column():
                ...
                easy_uploader.upload(
                    fn=lambda files, folder: [shutil.copy2(f.name, os.path.join(folder, os.path.split(f.name)[1])) for f in files] 
                    if folder != "" else gr.Warning('Please enter a folder name for your dataset'),
